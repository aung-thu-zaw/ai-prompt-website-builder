 ...
 * 1. Validates and normalizes the website configuration (generates slug if missing)
 ...
 *
 * The generated project is placed in `output/<slug>/` and archived as
 * `output/<slug>.zip`. The slug is automatically generated from
 * the website name if not explicitly provided, following URL-friendly conventions
 * (lowercase, hyphen-separated, special characters removed).
 *
 * @param {WebsiteSpec} spec - The complete website specification containing website name,
 *                             slug, description, architecture, theme, and pages.
...
export const generate = async (spec: WebsiteSpec): Promise<void> => {
  if (!spec.name || !spec.slug) {
    throw new Error("A valid website name and slug are required to proceed.");
  }

  // Generate project slug from name if not provided
  let projectSlug = spec.slug;
  if (!projectSlug) {
    if (!spec.name) {
      throw new Error(
        "A website slug or name must be provided for professional output."
      );
    }

    // Convert website name to URL-friendly slug: lowercase, hyphen-separated, special chars removed
    projectSlug = spec.name
      .toString()
      .trim()
      .toLowerCase()
      .replace(/[\s\W-]+/g, "-")
  ...
  const destDir = path.join(process.cwd(), "output", projectSlug);
  const zipFilePath = path.join(process.cwd(), "output", `${projectSlug}.zip`);
